...
interface Subagent {
  ...
  task: string;
  resultTokens: number;
  type: 'explore' | 'task' | 'plan';
  progress: number;
  internalTokens: number;
  maxInternalTokens: number;
  status: 'running' | 'complete';
  internalMessages: { text: string; tokens: number }[];
}
...
export default function ContextLoadingViz() {
  ...
  const addMultipleMessages = (count: number) => {
    if (compactionState === 'compacting' || hitLimit) return;
  ...
  // Spawn subagent
  const spawnSubagent = (type: 'explore' | 'task' | 'plan') => {
  ...
  const pct = (v: number) => ((v / maxTokens) * 100).toFixed(1);
  ...
  return (
    <div ref={conversationRef} className="min-h-screen bg-gray-950 text-gray-100 p-4 font-mono text-xs">
